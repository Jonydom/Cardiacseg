...
class Dataset_info:
    def __init__(self, dataset_dir, is_split=False):
        self.dataset_dir = dataset_dir
        self.train_images = self.get_train_images() if is_split else []
        self.train_labels = self.get_train_labels() if is_split else []
        self.test_images = self.get_test_images() if is_split else []
        self.test_labels = self.get_test_labels() if is_split else []
    ...
    """
    @staticmethod
    def split_dataset_v2(raw_dir, new_dir):
        os.makedirs(new_dir, exist_ok=True)
        os.makedirs(os.path.join(new_dir, "train/images"), exist_ok=True)
        os.makedirs(os.path.join(new_dir, "train/labels"), exist_ok=True)
        os.makedirs(os.path.join(new_dir, "val/images"), exist_ok=True)
        os.makedirs(os.path.join(new_dir, "val/labels"), exist_ok=True)
        os.makedirs(os.path.join(new_dir, "test/images"), exist_ok=True)
        os.makedirs(os.path.join(new_dir, "test/labels"), exist_ok=True)
...
if __name__ == "__main__":
    Dataset_info.split_dataset_v2(raw_dir="/root/autodl-tmp/ImageCHD", new_dir="/root/autodl-tmp/ImageCHD_split_sdf")