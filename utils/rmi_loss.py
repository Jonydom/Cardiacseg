...
class RMILoss_3D(nn.Module):
    ...
    def rmi_loss(self, pred, target):
        ...
        p_cov = p @ transpose(p)
        y_p_cov = y @ transpose(p)
#         print('y_conv: ', y_cov.shape, y_cov.max(), y_cov.min())
#         print('p_conv: ', p_cov.shape, p_cov.max(), p_cov.min())
#         print('y_p_conv: ', y_p_cov.shape, y_p_cov.max(), y_p_cov.min())
...
#         print('m: ', m.shape, m.max(), m.min())
        ...
        # Normalize
        rmi = rmi / float(vector_size)
#         print('rmi: ', rmi.shape, rmi.max(), rmi.min())
#         print(rmi)
    ...
    def extract_region_vector(self, x):
        ...
        x_regions = unfoldNd.UnfoldNd(kernel_size=self.radius, stride=stride)(x)
        x_regions = x_regions.view((*x.shape[:2], self.radius ** 3, -1))
        return x_regions
